...
export function OfflineManager() {
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (data.type === "status") {
        setStatus({ pages: data.pages, sprites: data.sprites, artwork: data.artwork });
      }
  ...
  return (
    <div className="offline-panel">
      <Row label="Seiten" bucket={status.pages} />
      ...
      <p className="offline-note">
        Seiten und Pixel-Bilder werden automatisch gespeichert. Lade zusätzlich das große Artwork
        herunter (am besten im WLAN), dann funktioniert der ganze Pokédex ohne Internet — auch im
        Flugzeug oder im Wald.
      </p>
    </div>
  );
}
